...
export const CoinAppView = ({ coin, mode, setMode, onUpdateCoins, onSave, onCancel }) => {
    return (
        <PopOutWidget label="Add / remove coins" onCancel={onCancel} onSave={onSave}>
            <div className={`flex items-center cursor-pointer h-full w-full ${tableBorderRounded}`} onClick={() => setMode(mode === 'add' ? 'subtr' : 'add')}>
                <div className={`text-4xl text-text-primary font-eskapade font-bold px-2 hover-glow rounded-l-md ${mode === 'add' ? 'bg-ic-luck/50' : 'bg-sheet-main-fill'}`}>+</div>
                <div className={`text-4xl text-text-primary font-eskapade font-bold px-2 hover-glow rounded-r-md ${mode === 'add' ? 'bg-sheet-main-fill' : 'bg-destructive-action/50'}`}>-</div>
            </div>
            <div className="flex gap-x-2">
                {
                    denominations.map(denomination => (
                        <div key={denomination} className={`
                                flex items-end text-2xl text-text-primary font-eskapade hover-glow
