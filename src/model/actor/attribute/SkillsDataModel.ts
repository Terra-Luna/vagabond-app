...
export const calculateDifficulties = (hero: HeroDataModel, skills: SkillsDataModel) => {
    ...
    skills.finesse.value = calculateSkill(Number(hero.stats.dexterity), skills.brawl.isTrained)
    skills.melee.value = calculateSkill(Number(hero.stats.might), skills.brawl.isTrained)
    skills.ranged.value = calculateSkill(Number(hero.stats.awareness), skills.brawl.isTrained)
    skills.arcana.value = calculateSkill(Number(hero.stats.reason), skills.brawl.isTrained)
    skills.craft.value = calculateSkill(Number(hero.stats.reason), skills.brawl.isTrained)
    skills.detect.value = calculateSkill(Number(hero.stats.awareness), skills.brawl.isTrained)
    skills.influence.value = calculateSkill(Number(hero.stats.presence), skills.brawl.isTrained)
    skills.medicine.value = calculateSkill(Number(hero.stats.reason), skills.brawl.isTrained)
    skills.mysticism.value = calculateSkill(Number(hero.stats.reason), skills.brawl.isTrained)
    skills.performance.value = calculateSkill(Number(hero.stats.presence), skills.brawl.isTrained)
    skills.sneak.value = calculateSkill(Number(hero.stats.dexterity), skills.brawl.isTrained)
    skills.survival.value = calculateSkill(Number(hero.stats.awareness), skills.brawl.isTrained)
}

function calculateSkill(stat: number, isTrained: boolean): number {
    const max = 20
    return isTrained ? max - stat * 2 : max - stat
}